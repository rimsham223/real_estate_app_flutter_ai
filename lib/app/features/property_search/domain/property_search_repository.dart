...
class PropertySearchRepository {
  ...
  Future<List<Area>> getAreas() async {
    try {
      final response = await _supabase.from('areas').select().order('name');
  ...
  Future<List<Compound>> getCompounds() async {
    try {
      final response = await _supabase.from('compounds').select('*, areas(*)').order('name');
  ...
  Future<List<PropertyType>> getPropertyTypes() async {
    try {
      final response = await _supabase.from('property_types').select().order('name');
      return response.map((json) => PropertyType.fromJson(json)).toList();
    } catch (_) {
  ...
  Future<List<Property>> searchProperties({
    ...
    try {
      ...
      final response = await query.order('min_price', ascending: true).limit(80);
